document.addEventListener('DOMContentLoaded', function() { 
    ...
    function handleSocketMessage(event) {
        ...
        if (event.data instanceof Blob) {
            ...
            // Define the onload handler to parse the Blob content as JSON
            ...
            // Handle non-Blob messages (assuming they are JSON)
            try {
    ...
    function processSocketMessage(message) {
        switch (message.type) {
            case "player_number":
                ...
                break;
            case "start_game":
                switchToAttackPhase(); // Assuming you start directly with the attack phase for simplification
                break;
            case "move":
                // Assuming message includes { player: 1 or 2, coordinates: { x, y }, hit: true/false }
                console.log(`Player ${message.player} attacked`, message.coordinates);
                if (message.player !== currentPlayer) {
                    // Update the board based on the attack
                    const attacks = message.player === 1 ? player2Attacks : player1Attacks;
                    attacks.push([message.coordinates.x, message.coordinates.y]);
                    // Update UI based on whether it was a hit or miss
                }
                switchTurns(); // Switch turns after processing an attack
                break;
            case "place_ship":
                // Similar handling for ship placements
                console.log("Ship placed by player", message.player, "at", message.coordinates);
                if (message.player !== currentPlayer) {
                    const ships = message.player === 1 ? player2Ships : player1Ships;
                    ships.push([message.coordinates.x, message.coordinates.y]);
    ...
    function sendShipPlacement(x, y) {
        const message = {
            type: 'place_ship',
